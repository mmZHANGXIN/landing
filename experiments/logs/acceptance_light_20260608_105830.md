...
- generated_at: `2026-06-08T10:58:30`
...
| DRL SB3 metadata | PASS | 0 | 22.8 | command exited 0 |
| GIS nine-grid lightweight contract | PASS | 0 | 17.2 | command exited 0 |
| GIS global-prior JSON acceptance self-test | PASS | 0 | 103.8 | command exited 0 |
| HALSS visualization lightweight contract | PASS | 0 | 26.6 | command exited 0 |
| visualization source contract | PASS | 0 | 15.4 | command exited 0 |
| depth projection lightweight contract | PASS | 0 | 13.9 | command exited 0 |
| SparseNet sparse convolution lightweight contract | PASS | 0 | 10.9 | command exited 0 |
| yaw-aware action decomposition | PASS | 0 | 15.4 | command exited 0 |
| runtime action-name labels | PASS | 0 | 21.4 | command exited 0 |
| field runbook source contract | FAIL | 1 | 16.0 | command exited 1 |
| field evidence status self-test | PASS | 0 | 775.3 | command exited 0 |
| Orin environment check self-test | PASS | 0 | 85.6 | command exited 0 |
| Orin environment report acceptance self-test | PASS | 0 | 95.6 | command exited 0 |
| requirement traceability source contract | PASS | 0 | 15.5 | command exited 0 |
| experiment requirement audit self-test | PASS | 0 | 180.8 | command exited 0 |
| field acceptance wrapper self-test | PASS | 0 | 79.1 | command exited 0 |
| closed-loop diagnostic snapshot contract | PASS | 0 | 16.8 | command exited 0 |
| MAVSDK home telemetry contract | PASS | 0 | 14.6 | command exited 0 |
| timing log acceptance self-test | PASS | 0 | 58.9 | command exited 0 |
| no-control log acceptance self-test | PASS | 0 | 218.8 | command exited 0 |
| no-control action diagnosis self-test | PASS | 0 | 103.2 | command exited 0 |
| no-control runtime contract self-test | PASS | 0 | 14.9 | command exited 0 |
| no-control artifact acceptance self-test | PASS | 0 | 174.8 | command exited 0 |
| DRL diagnosis JSON acceptance self-test | PASS | 0 | 83.4 | command exited 0 |
| SparseNet calibration JSON acceptance self-test | PASS | 0 | 102.3 | command exited 0 |
| saved binary semantic NPZ/PNG comparison self-test | PASS | 0 | 75.0 | command exited 0 |
| ROS topic evidence acceptance self-test | PASS | 0 | 59.9 | command exited 0 |
| flight-loop log acceptance self-test | PASS | 0 | 267.4 | command exited 0 |
| strict flight-ready gate regression | PASS | 0 | 753.3 | command exited 0 |
| flight-ready preflight preview | PASS | 1 | 26.2 | flight-ready preview has only expected field-configuration gaps |
| pipeline flight-ready check-only with GIS override | PASS | 0 | 37.0 | command exited 0 |
...
[OK] Config key present: depth_projection.backend
...
Preflight summary: failed=0, ...<truncated>
```
...
=== Lightweight GIS global-prior acceptance ===
  ...
  OK missing GIS traceability fails by default
  OK missing GIS traceability can be explicitly allowed
=== ALL PASSED ===
```
...
=== Lightweight depth projection acceptance ===
  ...
  OK single valid point is projected
  OK body axes map through R_I_to_C
  OK pose translation uses inverse transform
  OK z-buffer keeps nearest point
  OK invalid/behind/out-of-frame points stay at dmax
...
Traceback (most recent call last):
  File "/Users/evelyn/Desktop/orinlanding/orinlanding/test_runbook_contract_light.py", line 311, in <module>
    main()
  File "/Users/evelyn/Desktop/orinlanding/orinlanding/test_runbook_contract_light.py", line 298, in main
    test_acceptance_document_tracks_action_mapping_and_known_gaps()
  File "/Users/evelyn/Desktop/orinlanding/orinlanding/test_runbook_contract_light.py", line 162, in test_acceptance_document_tracks_action_mapping_and_known_gaps
    _require_all(
  File "/Users/evelyn/Desktop/orinlanding/orinlanding/test_runbook_contract_light.py", line 16, in _require_all
    assert not missing, f"{label} missing: {missing}"
AssertionError: EXPERIMENT_ACCEPTANCE action mapping and field gaps missing: ['Depth projection CPU path is budgeted']
```
...
=== Lightweight field evidence status acceptance ===
  ...
  OK no-control validator includes action diagnosis without safe-point arguments
=== ALL PASSED ===
```
...
```text
$ /Library/Developer/CommandLineTools/usr/bin/python3 /Users/evelyn/Desktop/orinlanding/orinlanding/test_audit_experiment_requirements_light.py
=== Lightweight experiment requirement audit ===
  OK local audit passes without claiming field completion
  OK strict-field fails until real evidence exists
  OK markdown audit report is written
=== ALL PASSED ===
```
...
=== Lightweight field acceptance wrapper ===
  ...
  OK dry-run passes explicit expected safe point
=== ALL PASSED ===
```
...
```text
$ /Library/Developer/CommandLineTools/usr/bin/python3 /Users/evelyn/Desktop/orinlanding/orinlanding/test_pipeline_diagnostic_snapshot_light.py
=== Lightweight pipeline diagnostic snapshot contract ===
  OK action-collapse snapshots keep HALSS binary semantic and sync evidence
  OK action-collapse snapshots keep yaw/action/velocity evidence
  OK global guidance waits for valid home telemetry before NED conversion
=== ALL PASSED ===
```
...
```text
$ /Library/Developer/CommandLineTools/usr/bin/python3 /Users/evelyn/Desktop/orinlanding/orinlanding/test_mavsdk_home_telemetry_light.py
=== Lightweight MAVSDK home telemetry contract ===
  OK MAVSDK tracks NED/GPS/attitude readiness and exposes wait_for_home
  OK zero GPS is rejected before global safe-point NED conversion
=== ALL PASSED ===
```
...
=== Lightweight no-control log acceptance ===
  ...
  OK cloud/odom sync budget fails
  OK yaw transform error fails
...
```text
$ /Library/Developer/CommandLineTools/usr/bin/python3 /Users/evelyn/Desktop/orinlanding/orinlanding/test_diagnose_nocontrol_action_log_light.py
=== Lightweight no-control action diagnosis ===
  OK legacy act=3(E), zero-yaw logs get actionable findings
  OK modern single-action logs point to live-frame DRL diagnosis
  OK multi-action yaw-fault logs pass
  OK one-class semantic observations are reported
=== ALL PASSED ===
```
...
```text
$ /Library/Developer/CommandLineTools/usr/bin/python3 /Users/evelyn/Desktop/orinlanding/orinlanding/test_nocontrol_runtime_contract_light.py
=== Lightweight no-control runtime contract acceptance ===
  OK --require-yaw-rate rejects zero yaw-fault runs before ROS init
  OK cloud/odom sync uses runtime.max_cloud_odom_sync_ms
  OK startup logs expose yaw rate, sync budget, and action mapping
=== ALL PASSED ===
```
...
=== Lightweight no-control artifact acceptance ===
  ...
  OK missing sync key fails
  OK stale cloud/odom sync fails
  OK yaw transform mismatch fails
  OK invalid action_id fails
=== ALL PASSED ===
```
...
=== Lightweight flight-loop log acceptance ===
  ...
  OK GIS prior safe-point mismatch fails
  OK GOTO_SAFE arrival error above tolerance fails
  OK near-zero GOTO_SAFE target fails
...
=== FlightReady gate regression tests ===
  ...
  OK fail: depth projection backend
...
2026-06-08 10:58:30,854 [INFO] OrinLanding: [FlightReady] Preview gates passed before model initialization
2026-06-08 10:58:30,854 [INFO] OrinLanding: [FlightReady] Check-only mode complete.
```
